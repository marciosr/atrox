...
use gtk::prelude::*;
use gtk::{
	glib, Adjustment, ApplicationWindow, Builder, Button, CheckButton, Entry, HeaderBar, Label,
	SpinButton,
};
use std::rc::Rc;
//use gtk::Inhibit;
...
impl AtroxUi {
	pub fn new() -> Rc<Self> {
		...
		let atroxui = Rc::new(Self {
			...
			spin_retorno,
			entry_tempo_conc,
			entry_coef_escorrimento,
			entry_intensidade_max,
			entry_vazao_bacia,
			entry_volume_enxurrada,
			entry_esp_h,
			entry_esp_v,
			entry_sessao,
			entry_vel_max,
	...
	pub fn run(&self, ui: Rc<AtroxUi>) {
		...
		ui2.bt_calcular.connect_clicked(move |_| {
			...
			let calculos = Calculos::calcula_terraco(
				valor,
			...
			ui.entry_coef_escorrimento
				.set_text(&coef_escorrimento.to_string());
			ui.entry_intensidade_max
				.set_text(&intens_max_chuva.to_string());
			ui.entry_vazao_bacia.set_text(&vazao_max.to_string());
			ui.entry_volume_enxurrada
				.set_text(&volume_enxurrada.to_string());
